"""
Job Service
Business logic for job management, extracted from routes to follow clean architecture.
"""
...
from app.models import JobResponse, DetailedProgress, SectionProgress
from app.services.infrastructure.storage import FileBasedJobRepository
...
from app.routes.jobs_helpers import (
...
class JobService:
    def __init__(self):
        self.repo = FileBasedJobRepository()
    ...
    def update_job(self, job_id: str, title: Optional[str] = None) -> JobResponse:
    ...
    def get_section_video_path(self, job_id: str, section_index: int) -> Path:
        """Get path to a section video."""
        if not validate_job_id(job_id):
            raise HTTPException(status_code=400, detail="Invalid job ID format")
    ...
    def list_completed_jobs(self) -> Dict[str, List[Dict[str, Any]]]:
        """List only completed jobs with videos."""
        jobs_list = self.repo.list_completed()
        seen_ids = set()
        completed = []
    ...
    def get_job_details(self, job_id: str) -> DetailedProgress:
        ...
        try:
            ...
            script = None
            script_progress = self._load_script_progress(job_id)
            if script_progress:
    ...
    def get_section_details(self, job_id: str, section_index: int) -> Dict[str, Any]:
        ...
        visual_description = self._load_visual_script(section, section_dir)
    ...
    def _load_visual_script(self, section: dict, section_dir: "Path") -> str:
        """
    ...
    def _load_script_progress(self, job_id: str) -> dict:
