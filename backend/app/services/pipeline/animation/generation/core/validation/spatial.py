...
INJECTED_METHOD = """
    def _perform_spatial_checks(self):
        ...
        for i, (idx1, t1) in enumerate(texts_with_idx):
            for (idx2, t2) in texts_with_idx[i+1:]:
                ...
                if is_overlapping(t1, t2):
                    ...
                    spatial_errors.append(f"Spatial Error: Text overlap detected between '{txt1[:10]}...' and '{txt2[:10]}...'.")
        ...
        for (t_idx, t) in texts_with_idx:
            # Cache family for performance?
            # get_family() is somewhat expensive.
            # But we only check if overlap detected.
            pass
            ...
            for (o_idx, o) in objects_with_idx:
                ...
                if o_idx > t_idx:
                     if is_overlapping(t, o):
                        ...
                        # Check ancestry
                        # If t is parent of o, or o is parent of t
                        # Using Manim's family check or simple containment if applicable
                        # Since we cannot easily check containment in flat list, we rely on Manim methods if available
                        # or skip if names match closely?
                        # Re-fetching family is safest.
                        ...
                        spatial_errors.append(f"Spatial Error: Object '{type(o).__name__}' obscures text '{txt[:10]}...'.")
        ...
        if spatial_errors:
             ...
             if len(joined_errors) > 500: joined_errors = joined_errors[:500] + "..."
             sys.exit(f"Spatial Validation Failed ({error_count} errors): {joined_errors}")
"""
