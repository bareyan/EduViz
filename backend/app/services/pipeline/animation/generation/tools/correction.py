...
class CorrectionToolHandler:
    ...
    def _apply_fixes(self, code: str, fixes: List[Dict]) -> tuple:
        ...
        for fix in fixes:
            ...
            if not search:
                details.append("WARN Skipped: empty search text")
                continue
            ...
            if search in new_code:
                ...
                if count == 1:
                    ...
                    applied += 1
                    details.append(f"OK Applied: {reason}" if reason else "OK Applied fix")
                else:
                    details.append(f"WARN Skipped: search text appears {count} times (must be unique)")
            else:
                details.append("FAIL Not found: search text not in code")
